...
# Cloudflare 官方回源网段（https://www.cloudflare.com/ips/，如官方调整请同步更新）
_CF_NETWORKS = [ipaddress.ip_network(n) for n in (
...
def _is_cloudflare_ip(ip):
    try:
        ...
        return False
    return any(a in n for n in _CF_NETWORKS)


def _client_ip():
    """Agent 真实来源 IP（决定测速目标可达性的关键）：
    - remote 是 Cloudflare 网段（面板直挂 CF，无 Caddy）：CF-Connecting-IP 由 CF
      强制写入、不可伪造，最可信；
    - remote 是内网（Caddy 反代后面）：Caddy 已用 header_up 把 XFF 覆盖为对端
      IP，取 XFF 最后一项；同时 Caddy 会剥掉直连方自带的 CF-Connecting-IP；
    - 公网直连：一律用 remote_addr，防止伪造头伪造 agent_ip / 绕过限速。"""
    ra = request.remote_addr or ''
    ip = ra
    try:
        ra_ip = ipaddress.ip_address(ra)
        behind_proxy = ra_ip.is_private or _is_cloudflare_ip(ra)
    except ValueError:
        behind_proxy = False
    if behind_proxy:
