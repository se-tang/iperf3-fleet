#!/bin/bash
# 模拟 agent：本地调试面板任务管道用（与 app/agent/install.sh 里的真实 agent 协议一致）
# 用法: bash tests/fake_agent.sh <接入令牌> [面板地址] [运行秒数]
PANEL_URL="${2:-http://127.0.0.1:8000}"
TOKEN="${1:?用法: fake_agent.sh <token> [面板地址] [秒数]}"
DURATION="${3:-90}"
SPOOL=$(mktemp -d)
...
echo "fake agent 启动 token=${TOKEN:0:8}... 运行 ${DURATION}s"
...
while [ $SECONDS -lt $end ]; do
  ...
  timeout=${timeout:-120}
  printf '%s\n' "$resp" | sed -n 's/^cmd_b64=//p' | base64 -d > "$SPOOL/job.sh"
